...
(function ($) {
  ...
  function scrolltoElement(a, correction) {
    ...
    $("html, body").animate(
      { scrollTop: $(a.getAttribute("href")).offset().top - correction },
      400
    );
  }
  ...
  function setTocToggle() {
    ...
    $toc.on("click", "a", (e) => {
      ...
      e.stopPropagation();
      scrolltoElement(
        e.target.tagName.toLowerCase === "a" ? e.target : e.target.parentElement
      );
    });
  ...
  function getHitokoto() {
    ...
    if (BANNER_HITOKOTO) {
      ...
      const url = "https://v1.hitokoto.cn/?c=c";
      $("body").append('<script	src="%s"></script>'.replace("%s", url));
      window.handlerHitokoto = (data) => {
        $hitokoto.css("color", "transparent").text(data.hitokoto);
        if (data.source)
          $hitokoto.append("<cite> ——  %s</cite>".replace("%s", data.source));
        else if (data.author)
          $hitokoto.append("<cite> ——  %s</cite>".replace("%s", data.author));
